...
import { deleteExpense, getExpenses, updateExpense } from "@/actions/expense";
import { expenseType } from "@/utils/schema";
import formatDate from "@/utils/formateDate";
import { useExpenses } from "@/store";
import { toast } from "sonner";

export function DataTable({ budgetId }: { budgetId: string }) {
  const expenses = useExpenses((state) => state.expenses);
  const setExpenses = useExpenses((state) => state.setExpenses);
  const removeExpense = useExpenses((state) => state.deleteExpense);
        ...
        <form id="edit-expense-form" onSubmit={handleEditFormSubmit}>
          <DialogContent className="sm:max-w-sm">
            ...
            <DialogFooter>
              ...
              <Button form="edit-expense-form" disabled={isDisabled || isUpdating} type="submit">
                {isUpdating ? (
                  <>
